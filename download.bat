...
if /i "%~1"=="--setup" (
  if exist "%~dp0setup.ps1" (
    powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0setup.ps1" -SkipPath
    exit /b !errorlevel!
  ) else (
    echo setup.ps1 missing — run setup.bat from a full clone
    exit /b 1
  )
)
...
:main
set "ESC="
if defined NO_COLOR goto nocolor
if "%TERM%"=="dumb" goto nocolor
for /F %%? in ('echo prompt $E^|cmd') do set "ESC=%%?"
if not defined ESC goto nocolor
set "R=%ESC%[0m"
...
if not "!LATEST!"=="5.0.0" (
  if "!LATEST!"=="" (
    ...
    echo   %FAINT%downloading into this folder ...%R%
    curl.exe -L --max-time 120 -o "downterm.zip" "https://github.com/onion3130/downterm/archive/refs/tags/!LATEST!.zip" 2>nul
    if exist "downterm.zip" (
      powershell -NoProfile -ExecutionPolicy Bypass -Command "& { \$p=(Get-Location).Path; Expand-Archive -Force -Path (Join-Path \$p 'downterm.zip') -DestinationPath (Join-Path \$p '_dl'); \$s=Get-ChildItem (Join-Path \$p '_dl') -Directory | Select-Object -First 1; Copy-Item -Force (Join-Path \$s.FullName 'download.bat'),(Join-Path \$s.FullName 'filter.ps1'),(Join-Path \$s.FullName 'setup.ps1') -Destination \$p; Remove-Item -Recurse -Force (Join-Path \$p '_dl'),(Join-Path \$p 'downterm.zip') }"
      echo   %GOOD%wrapped scripts updated to !LATEST!. restart downterm.%R%
    ) else (
      echo   %BAD%download failed — grab it from the releases page.%R%
    )
  )
) else (
