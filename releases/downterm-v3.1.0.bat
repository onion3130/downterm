...
rem downterm 3.1.0 — minimal terminal UI only (no window GUI)
rem   downterm            → menu
rem   downterm --version
rem   downterm --setup
rem   downterm --install  → add this folder to user PATH
...
cls
echo.
echo   %ACC%downterm%R%  %FAINT%3.1.0%R%
echo   %HAIR%..........................................%R%
echo.
...
echo   %INK%7%R%  %FAINT%install PATH  ^(type downterm anywhere^)%R%
...
cls
echo.
echo   %ACC%install PATH%R%
echo   %HAIR%..........................................%R%
echo.
echo   %MUT%so you can type%R%  %INK%downterm%R%  %MUT%in any terminal%R%
echo.
echo   %FAINT%folder:%R%
echo   %FAINT%%~dp0%R%
echo.
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$dir = (Resolve-Path '%~dp0').Path.TrimEnd('\');" ^
  "$name = 'Path'; $scope = 'User';" ^
  ...
  "if ($norm -contains $dir) {" ^
  "  Write-Host '  already on PATH';" ^
  "  exit 0" ^
  "};" ^
  "$new = if ($cur.Trim() -eq '') { $dir } else { $cur.TrimEnd(';') + ';' + $dir };" ^
  "[Environment]::SetEnvironmentVariable($name, $new, $scope);" ^
...
if errorlevel 1 (
  ...
  echo.
  echo   %GOOD%ok.%R%  %FAINT%open a NEW terminal, then type:%R%
  echo.
  echo     %INK%downterm%R%
  echo.
  echo   %FAINT%^(this window already open keeps the old PATH^)%R%
)
echo.
echo   %FAINT%any key...%R%
pause>nul
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if (-not $cur) { exit 0 };" ^
  "$parts = $cur -split ';' | Where-Object { $_ -and ($_.TrimEnd('\') -ne $dir) };" ^
...
cls
echo.
echo   %ACC%help%R%  %FAINT%3.1.0%R%
echo   %HAIR%..........................................%R%
echo.
echo   %MUT%terminal only — no separate window app%R%
echo.
echo   1. copy a video link in your browser
echo   2. run downterm  ^(or this menu^)
echo   3. press %INK%1%R% for best video
echo.
echo   %MUT%PATH%R%
echo     press %INK%7%R% once → new terminals get  downterm
echo.
echo   %MUT%keys%R%
echo     1 best video   2 quality   3 audio
echo     4 history      5 folder    6 setup
echo     7 PATH install 8 help      9 quit
echo.
echo   %FAINT%any key...%R%
pause>nul
goto menu
...
echo.
echo   %GOOD%done.%R%
echo   %FAINT%optional: press 7 to install PATH%R%
pause>nul
goto menu
