...
rem downterm 3.1.1 — minimal terminal UI only (no window GUI)
rem   downterm            → menu
...
cls
echo.
echo   %ACC%downterm%R%  %FAINT%3.1.1%R%
echo   %HAIR%..........................................%R%
echo.
...
echo   %FAINT%%~dp0%R%
echo.
if not exist "%~dp0downterm.cmd" (
  > "%~dp0downterm.cmd" echo @echo off
  >> "%~dp0downterm.cmd" echo cd /d "%%~dp0"
  >> "%~dp0downterm.cmd" echo call "%%~dp0download.bat" %%*
)
...
if errorlevel 1 (
  ...
  echo   %GOOD%done.%R%
  echo   %FAINT%open a NEW PowerShell or cmd window, then type downterm%R%
)
echo.
echo   %FAINT%any key...%R%
pause>nul
...
echo.
echo   %MUT%make "downterm" work anywhere%R%
echo     press %INK%7%R% once  ^(or: download.bat --install^)
echo     open a new terminal → type:  downterm
echo.
echo   %MUT%keys%R%
