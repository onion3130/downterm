#!/usr/bin/env bash
# downterm 3.1.0 — minimal terminal UI (numbers only). no window GUI.
set -u
ESC="\033"
R="${ESC}[0m"
MUT="${ESC}[38;5;145m"
...
case "${1:-}" in
  --version) echo "downterm 3.1.0"; exit 0 ;;
  --install)
    ...
    printf "  ensure ~/.local/bin is on PATH, then open a new shell and type: downterm\n"
    exit 0
    ;;
  --uninstall)
...
menu() {
  while true; do
    ...
    printf "  ${INK}7${R}  install PATH  (type downterm anywhere)\n"
    printf "  ${INK}8${R}  help\n"
    printf "  ${INK}9${R}  quit\n\n  ${MUT}>${R} "
